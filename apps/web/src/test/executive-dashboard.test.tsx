...
describe('executive economic dashboard',()=>{beforeEach(()=>vi.mocked(dashboard.loadExecutiveDashboard).mockResolvedValue(data as never))
 it('loads latest KPIs and all decision charts',async()=>{renderPage();expect(await screen.findByText('PIB total')).toBeInTheDocument();expect(screen.getByTestId('gdp-structure-chart')).toBeInTheDocument();expect(screen.getByTestId('gdp-growth-chart')).toBeInTheDocument();expect(screen.getByTestId('trade-balance-chart')).toBeInTheDocument();expect(screen.getByTestId('sector-evolution-chart')).toBeInTheDocument();expect(screen.getByTestId('gdp-without-mining-chart')).toBeInTheDocument();expect(screen.getByTestId('sector-share-donut')).toBeInTheDocument();expect(screen.getByTestId('sector-growth-heatmap')).toBeInTheDocument()})
